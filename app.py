...
def bereken_micro_stats(goals_t, goals_u, exp_goals_t, exp_goals_u):
    base_shots_t = 7 + (exp_goals_t * 2.5) + (goals_t * 1.5)
    base_shots_u = 7 + (exp_goals_u * 2.5) + (goals_u * 1.5)
    
    schot_t = max(goals_t, int(random.gauss(base_shots_t, 2)))
    schot_u = max(goals_u, int(random.gauss(base_shots_u, 2)))
    
    sot_t = goals_t + int((schot_t - goals_t) * random.uniform(0.25, 0.40))
    sot_u = goals_u + int((schot_u - goals_u) * random.uniform(0.25, 0.40))
...
def simuleer_doelpunten(lambda_basis, temming=True):
    lambda_basis = max(0.01, min(5.0, lambda_basis)) 
    ...
    # Temming minder streng gemaakt (was > 0.10, nu > 0.40) voor extremere maar realistische voetbaluitslagen.
    if temming and goals >= 5: 
        if random.random() > 0.40: 
            goals = random.choice([3, 4])
...
def simuleer_wedstrijd_90min(thuis_team, uit_team, handmatige_odd_thuis=None, handmatige_odd_uit=None):
    ...
    # xG defaults verhoogd voor realistischere scores
    xg_t = float(thuis_team.get('xG last 10', 1.45)) if pd.notna(thuis_team.get('xG last 10')) else 1.45
    xga_t = float(thuis_team.get('xGA last 10', 1.15)) if pd.notna(thuis_team.get('xGA last 10')) else 1.15
    xg_u = float(uit_team.get('xG last 10', 1.45)) if pd.notna(uit_team.get('xG last 10')) else 1.45
    xga_u = float(uit_team.get('xGA last 10', 1.15)) if pd.notna(uit_team.get('xGA last 10')) else 1.15
...
def run_enkel_toernooi(df_teams):
    ...
    # xG defaults ook hier verhoogd in team-initialisatie
    standen = {row['Land']: {"Land": row['Land'], "Poule": row['Poule'], "Elo": row['Elo'], "Vorm_Multiplier": row['Vorm_Multiplier'], "Formatie": row['Formatie'], "Odd": row['Odd'], "FC26": row['FC26'], "Marktwaarde": row['Marktwaarde'], "xG last 10": row.get('xG last 10', 1.45), "xGA last 10": row.get('xGA last 10', 1.15), "Pnt": 0, "DS": 0, "DV": 0, "DT": 0} for _, row in df_teams.iterrows()}
    groeps_uitslagen = {}
    
    fifa_schema = [(0,1), (2,3), (0,2), (3,1), (3,0), (1,2)]

    for poule in poules:
        teams = df_teams[df_teams['Poule'] == poule].reset_index(drop=True)
        for t_idx, u_idx in fifa_schema:
            t, u = teams.iloc[t_idx], teams.iloc[u_idx]
            gt, gu = simuleer_wedstrijd_90min(standen[t['Land']], standen[u['Land']])
            groeps_uitslagen[f"{t['Land']} vs {u['Land']}"] = f"{gt}-{gu}"
            standen[t['Land']]["DV"] += gt; standen[t['Land']]["DT"] += gu
            standen[u['Land']]["DV"] += gu; standen[u['Land']]["DT"] += gt
            if gt > gu: standen[t['Land']]["Pnt"] += 3
            elif gu > gt: standen[u['Land']]["Pnt"] += 3
            else: standen[t['Land']]["Pnt"] += 1; standen[u['Land']]["Pnt"] += 1
...
def run_match_simulation(thuis_row, uit_row, odd1, oddX, odd2, runs=5000):
    # PURE POISSON SIMULATIE
    tot_impl = (1/odd1) + (1/oddX) + (1/odd2)
    p_t = ((1/odd1) / tot_impl)
    p_x = ((1/oddX) / tot_impl)
    p_u = ((1/odd2) / tot_impl)
    
    # xG verhoogd voor de losse matches
    xg_t = float(thuis_row.get('xG last 10', 1.45)) if pd.notna(thuis_row.get('xG last 10')) else 1.45
    xga_t = float(thuis_row.get('xGA last 10', 1.15)) if pd.notna(thuis_row.get('xGA last 10')) else 1.15
    xg_u = float(uit_row.get('xG last 10', 1.45)) if pd.notna(uit_row.get('xG last 10')) else 1.45
    xga_u = float(uit_row.get('xGA last 10', 1.15)) if pd.notna(uit_row.get('xGA last 10')) else 1.15

    tempo, mod_t, mod_u = bereken_formatie_impact(thuis_row['Formatie'], uit_row['Formatie'])

    base_lam_t = ((xg_t + xga_u) / 2.0) * tempo * mod_t
    base_lam_u = ((xg_u + xga_t) / 2.0) * tempo * mod_u

    # Boost toegevoegd (+10%) op de totale expected goals om minimale uitslagen af te zwakken
    match_xg = (base_lam_t + base_lam_u) * 1.10
    
    lam_t = max(0.5, min(4.0, match_xg * (p_t + (1 - p_t - p_u)*0.5)))
    lam_u = max(0.5, min(4.0, match_xg * (p_u + (1 - p_t - p_u)*0.5)))
    ...
    for _ in range(runs):
        gt = simuleer_doelpunten(lam_t, temming=True)
        gu = simuleer_doelpunten(lam_u, temming=True)
            
        custom_scores[f"{gt} - {gu}"] += 1
        
        shots_t, sot_t, saves_t, shots_u, sot_u, saves_u = bereken_micro_stats(gt, gu, lam_t, lam_u)
        tot_shots_t += shots_t; tot_sot_t += sot_t; tot_saves_t += saves_t
        tot_shots_u += shots_u; tot_sot_u += sot_u; tot_saves_u += saves_u
        
    avg_stats = { 
        "shots_t": round(tot_shots_t/runs, 1), "sot_t": round(tot_sot_t/runs, 1), "saves_t": round(tot_saves_t/runs, 1), 
        "shots_u": round(tot_shots_u/runs, 1), "sot_u": round(tot_sot_u/runs, 1), "saves_u": round(tot_saves_u/runs, 1)
    }
    
    return custom_scores, avg_stats
...
if fase == "Database & Teams":
    ...
    if st.button("🚀 Start Wereldkampioenschap"):
        poules = df_teams['Poule'].unique()
        standen = {row['Land']: {"Land": row['Land'], "Poule": row['Poule'], "Elo": row['Elo'], "Vorm_Multiplier": row['Vorm_Multiplier'], "Formatie": row['Formatie'], "Odd": row['Odd'], "FC26": row['FC26'], "Marktwaarde": row['Marktwaarde'], "xG last 10": row.get('xG last 10', 1.45), "xGA last 10": row.get('xGA last 10', 1.15), "GS": 0, "W": 0, "G": 0, "V": 0, "DV": 0, "DT": 0, "DS": 0, "Pnt": 0} for _, row in df_teams.iterrows()}
    ...
    st.write("Simuleer het WK duizenden keren op de achtergrond. Krachtverhoudingen zijn gebaseerd op de robuuste dataset.")
    ...
    if st.button("🔮 Start Monte Carlo Simulatie"):
        ...
        with col_rechts:
            st.subheader("⚽ Meest Voorkomende Groepsfase Scores")
            score_data = []
            for match, counter in wedstrijd_scores.items():
                ...
                score_data.append({"Wedstrijd": match, "Meest Waarschijnlijke Uitslag": meest_voorkomende_score, "Kans (%)": percentage})
    ...
    if st.button("🚀 Genereer Scorito tips voor poule"):
        for key, data in match_inputs.items():
            scores, avg_stats = run_match_simulation(data['t'], data['u'], data['odd1'], data['oddX'], data['odd2'], runs=5000)
    ...
    if st.button("📊 Bereken Score & Stats"):
        custom_scores, avg_stats = run_match_simulation(t_row, u_row, o1, oX, o2, runs=5000)
        ...
        # --- UITSLAGENMATRIX (HEATMAP) PURE POISSON ---
        st.write("---")
        st.subheader("🔥 Uitslagenmatrix (Kans op exacte eindstand)")
        st.markdown(f"**Rijen (↓):** Doelpunten **{t_naam}** | **Kolommen (→):** Doelpunten **{u_naam}**")
        
        matrix_data = [[0.0 for _ in range(6)] for _ in range(6)]
        
        for score_str, count in custom_scores.items():
            try:
                parts = score_str.split('-')
                gt = int(parts[0].strip())
                gu = int(parts[1].strip())
                
                gt_idx = min(gt, 5)
                gu_idx = min(gu, 5)
                
                matrix_data[gt_idx][gu_idx] += count
            except:
                pass
                
        for r in range(6):
            for c in range(6):
                matrix_data[r][c] = round((matrix_data[r][c] / 5000) * 100, 1)
                
        labels = ["0", "1", "2", "3", "4", "5+"]
        df_matrix = pd.DataFrame(matrix_data, index=labels, columns=labels)
        
        st.dataframe(
            df_matrix.style.background_gradient(cmap="YlOrRd", axis=None).format("{:.1f}%"),
